...
typedef struct{
  ...
  uint16_t  CustomReboot_Request_ChHdle;                  /**< REBOOT_REQUEST_CHAR handle */
  uint16_t  CustomDelta_T_ChHdle;                  /**< DELTA_T_CHAR handle */
...
uint16_t SizeReboot_Request_Ch = 3;
uint16_t SizeDelta_T_Ch = 4;
...
#define COPY_REBOOT_REQUEST_CHAR_UUID(uuid_struct)    COPY_UUID_128(uuid_struct,0x00,0x00,0xfe,0x11,0x8e,0x22,0x45,0x41,0x9d,0x4c,0x21,0xed,0xae,0x82,0xed,0x19)
#define COPY_DELTA_T_CHAR_UUID(uuid_struct)    COPY_UUID_128(uuid_struct,0x00,0x00,0xfe,0x43,0x8e,0x22,0x45,0x41,0x9d,0x4c,0x21,0xed,0xae,0x82,0xed,0x19)
...
void SVCCTL_InitCustomSvc(void)
{
   ...
   *                                2 for REBOOT_REQUEST_CHAR +
   *                                2 for DELTA_T_CHAR +
   *                                1 for My_Switch_Char configuration descriptor +
   *                                1 for DELTA_T_CHAR configuration descriptor +
   *                                1 for DELTA_T_CHAR broadcast property +
   *                              = 12
   *
  ...
  ret = aci_gatt_add_char(CustomContext.CustomP2PsHdle,
                          UUID_TYPE_128, &uuid,
                          SizeReboot_Request_Ch,
                          CHAR_PROP_WRITE_WITHOUT_RESP,
                          ATTR_PERMISSION_NONE,
                          GATT_NOTIFY_ATTRIBUTE_WRITE,
                          0x10,
                          CHAR_VALUE_LEN_VARIABLE,
                          &(CustomContext.CustomReboot_Request_ChHdle));
  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail   : aci_gatt_add_char command   : REBOOT_REQUEST_CH, error code: 0x%x \n\r", ret);
  }
  else
  {
    APP_DBG_MSG("  Success: aci_gatt_add_char command   : REBOOT_REQUEST_CH \n\r");
  }
  ...
  ret = aci_gatt_add_char(CustomContext.CustomP2PsHdle,
                          ...
                          0x10,
                          CHAR_VALUE_LEN_VARIABLE,
                          &(CustomContext.CustomDelta_T_ChHdle));
  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail   : aci_gatt_add_char command   : DELTA_T_CH, error code: 0x%x \n\r", ret);
  }
  else
  {
    APP_DBG_MSG("  Success: aci_gatt_add_char command   : DELTA_T_CH \n\r");
  }
...
tBleStatus Custom_STM_App_Update_Char(Custom_STM_Char_Opcode_t CharOpcode, uint8_t *pPayload)
{
  ...
  switch (CharOpcode)
  {
    ...
    case CUSTOM_STM_REBOOT_REQUEST_CH:
      ret = aci_gatt_update_char_value(CustomContext.CustomP2PsHdle,
                                       CustomContext.CustomReboot_Request_ChHdle,
                                       0, /* charValOffset */
                                       SizeReboot_Request_Ch, /* charValueLen */
                                       (uint8_t *)  pPayload);
      if (ret != BLE_STATUS_SUCCESS)
      {
        APP_DBG_MSG("  Fail   : aci_gatt_update_char_value REBOOT_REQUEST_CH command, result : 0x%x \n\r", ret);
      }
      else
      {
        APP_DBG_MSG("  Success: aci_gatt_update_char_value REBOOT_REQUEST_CH command\n\r");
      }
    ...
    case CUSTOM_STM_DELTA_T_CH:
      ret = aci_gatt_update_char_value(CustomContext.CustomP2PsHdle,
                                       CustomContext.CustomDelta_T_ChHdle,
                                       0, /* charValOffset */
                                       SizeDelta_T_Ch, /* charValueLen */
                                       (uint8_t *)  pPayload);
      if (ret != BLE_STATUS_SUCCESS)
      {
        APP_DBG_MSG("  Fail   : aci_gatt_update_char_value DELTA_T_CH command, result : 0x%x \n\r", ret);
      }
      else
      {
        APP_DBG_MSG("  Success: aci_gatt_update_char_value DELTA_T_CH command\n\r");
      }
...
tBleStatus Custom_STM_App_Update_Char_Ext(uint16_t Connection_Handle, Custom_STM_Char_Opcode_t CharOpcode, uint8_t *pPayload)
{
  ...
  switch (CharOpcode)
  {
    ...
    case CUSTOM_STM_REBOOT_REQUEST_CH:
      ...
      ret = Generic_STM_App_Update_Char_Ext(Connection_Handle, CustomContext.CustomP2PsHdle, CustomContext.CustomReboot_Request_ChHdle, SizeReboot_Request_Ch, pPayload);
    ...
    case CUSTOM_STM_DELTA_T_CH:
      ...
      ret = Generic_STM_App_Update_Char_Ext(Connection_Handle, CustomContext.CustomP2PsHdle, CustomContext.CustomDelta_T_ChHdle, SizeDelta_T_Ch, pPayload);
